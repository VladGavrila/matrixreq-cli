...
const releaseURL = "https://api.github.com/repos/VladGavrila/matrixreq-cli/releases/latest"
...
func Run(currentVersion string) error {
	...
	tmpFile, err := os.CreateTemp(dir, ".mxreq-upgrade-*")
	if err != nil {
	...
	if err := os.Rename(tmpPath, binaryPath); err != nil {
		return fmt.Errorf("failed to replace binary (try: sudo mxreq --upgrade): %w", err)
	}

	// 7. Success.
	fmt.Printf("Updated mxreq to v%s\n", latestVersion)
	return nil
}
...
func assetName() (string, error) {
	...
	case "darwin/arm64":
		return "mxreq-macos-arm64", nil
	case "linux/amd64":
		return "mxreq-linux-amd64", nil
	default:
