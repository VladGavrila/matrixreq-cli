...
1. Run `mxreq project tree -p <project> --filter <CATEGORY> -o json` to get the project tree for the relevant category (e.g. SOFT, SPEC).
2. Search the tree output for a folder whose title/label matches the chosen name.
3. **If found:** use its folder reference (e.g. `F-SOFT-42`) for all items.
...
| `item_ref` | string | No | Existing item ID (e.g. `SOFT-42`) — include ONLY when updating an existing item. Omit entirely for new items. |
| `folder` | string | No | Target folder reference (e.g. `F-SOFT-1`, `F-SPEC-10`). Format: `F-<CATEGORY>-<NUMBER>` |
...
8. If the user specifies a category (SOFT, SPEC, RISK, etc.), use it consistently; otherwise default to SOFT
...
2. Run `mxreq project tree -p PROJ --filter SOFT -o json` → no "MXREQ" folder found
3. Run `mxreq folder create -p PROJ --parent F-SOFT-1 --label "MXREQ" -r "Auto-created folder for requirements"` → Created folder ID=42
4. Use `F-SOFT-42` as folder for all items
5. Output:
