#!/usr/bin/env bash
# Build, sign, and notarize mxreq CLI binaries.
#
...
if [[ -n "${DEVELOPER_ID_CERT_BASE64:-}" ]]; then
    KEYCHAIN_PATH="$TMPDIR/mxreq-build.keychain"
    ...
    echo "$DEVELOPER_ID_CERT_BASE64" | base64 --decode > "$TMPDIR/mxreq-cert.p12"
    security import "$TMPDIR/mxreq-cert.p12" \
        -k "$KEYCHAIN_PATH" \
